...
const useState = (initial) => {
  let currentFiber = wipFiber;
  const oldHooks = currentFiber.alternate?.stateHooks[stateHooksIndex];
  const stateHook = {
    state: oldHooks ? oldHooks.state : initial,
    quene: oldHooks ? oldHooks.quene : [],
  };
  stateHook.quene.forEach((action) => {
  ...
  const setState = (action) => {
    ...
    stateHook.quene.push(isFunction ? action : () => action);
    wipRoot = {
      ...currentFiber,
      alternate: currentFiber,
    };
